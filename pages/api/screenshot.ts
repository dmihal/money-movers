...
async function getScreenshot(): Promise<any> {
  ...
  const request = await fetch(`https://api.browshot.com/api/v1/screenshot/create?url=https://money-movers.info/&instance_id=12&size=screen&cache=${60 * 60 * 24}&key=${key}`)
